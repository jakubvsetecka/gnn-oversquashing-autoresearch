...
Current variant: pure attention retrieval, HIDDEN=128.
Width raised 64 -> 128 so the query/key space can separate the 2^7 = 128
distinct keys at radius 7 (representation capacity must scale with the
information content that must pass the bottleneck). Otherwise: embed ->
single global self-attention layer -> readout at root, no tree layers.
"""
