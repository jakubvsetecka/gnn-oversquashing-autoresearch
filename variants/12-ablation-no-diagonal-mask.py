...
Current variant: ABLATION - raw-feature-bias attention WITHOUT diagonal mask.
Tests whether masking self-attention is necessary: root's self-similarity is
2 (query + is_root flags) vs 1 for the matching leaf, so without the mask the
softmax should be dominated by the root's own embedding, starving retrieval.
"""
