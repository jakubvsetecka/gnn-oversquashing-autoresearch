...
Current variant: ABLATION - GAT-style local attention on tree edges only.
Baseline topology and depth (r+1 layers, root readout), but each layer uses
scaled dot-product attention masked to tree edges + self instead of fixed
degree normalization. Tests whether ADAPTIVE edge weighting alone (no global
shortcut) mitigates over-squashing: attention can route relevant messages but
cannot expand the fixed-width bottleneck.
"""
...
class GCN(nn.Module):
    def __init__(self, r: int):
        super().__init__()
        n = 2 ** (r + 1) - 1
        mask = (tree_adjacency(r) + torch.eye(n)) > 0
        self.register_buffer("edge_mask", mask)
        self.q = nn.ModuleList(nn.Linear(HIDDEN, HIDDEN) for _ in range(r + 1))
        self.k = nn.ModuleList(nn.Linear(HIDDEN, HIDDEN) for _ in range(r + 1))
    ...
    def forward(self, X):
        h = self.embed(X)
        for q, k, lin in zip(self.q, self.k, self.layers):
            logits = (q(h) @ k(h).transpose(-2, -1)) / HIDDEN**0.5
            logits = logits.masked_fill(~self.edge_mask, float("-inf"))
            h = F.relu(lin(torch.softmax(logits, dim=-1) @ h))
